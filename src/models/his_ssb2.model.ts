...
export class HisSsb2Model {
  ...
  async getAllergyDetail(db: Knex, hn: any) {
    let data = await db.raw(`select SUBSTRING(SSBSTOCK.dbo.STOCK_MASTER.ENGLISHNAME,2,LEN(SSBSTOCK.DBO.STOCK_MASTER.ENGLISHNAME)) as drug_name,
  ...
  async getDrugs(db: Knex, hn: any, dateServe: any) {
    let data = await db.raw(`SELECT VNMST.hn+''+ VNMST.VN as seq,
	    ...
	    ISNULL(SUBSTRING(SSBSTOCK.DBO.STOCK_MASTER.ENGLISHNAME,2,LEN(SSBSTOCK.DBO.STOCK_MASTER.ENGLISHNAME)),
	    SUBSTRING(SSBSTOCK.DBO.STOCK_MASTER.THAINAME,2,LEN(SSBSTOCK.DBO.STOCK_MASTER.THAINAME))) as drug_name,
	    ...
	    from VNMEDICINE 
	    inner join SSBSTOCK.DBO.STOCK_MASTER on VNMEDICINE.stockcode = SSBSTOCK.DBO.STOCK_MASTER.STOCKCODE
